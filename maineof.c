...
/*   Updated: 2019/12/02 11:35:24 by seruiz      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int	main(void)
{
	...
	fd[7] = open("test/fewchar_perline.txt", O_RDONLY);
	fd[5] = open("test/long_line.txt", O_RDONLY);
	fd[6] = open("test/normal.txt", O_RDONLY);
	fd[4] = open("test/only_newline.txt", O_RDONLY);
	...
	while (i <= 7)
	{
		...
		if ((ret = get_next_line(fd[i], &buff)) >= 0)
		{
			printf("resut : %s\n ret : %d\n\n", buff, ret);
			free(buff);
		}
